...
public class UserRegister {
	...
	public int Register() throws Exception {
		...
		PreparedStatement pst = conn
				.prepareStatement("INSERT INTO UserRegister (username, emailid, password) values (?, ?, ?)");
	...
	public List<Map<String, Object>> queryByUserName() throws Exception {
		PreparedStatement pst = conn.prepareStatement("SELECT userid, EmailId from UserRegister where username=?");
		pst.setString(1, userName);
		return getQuery(pst);
	}

	public List<Map<String, Object>> queryByEmailId() throws Exception {
		PreparedStatement pst = conn.prepareStatement("SELECT userid, username from UserRegister where emailid=?");
